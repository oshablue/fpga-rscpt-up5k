  ...
  `ifdef SELFREAD
    ...
    begin
      if ( trigd | !new_capt ) begin
        dumpclkreg <= `EXT_FIFO_WR_IDLE; // EXT_FIF)_WR_IDLE is = 1 thus //1'b 0;
      end
...
`endif
  ...
  `endif
    ...
    // READ address increment/reset
    // DELETED - TESTING 04-26-2020 -- latches created without the identity?  Or because of it?
    // NOPE: Doing this basically kills data output
    always @( posedge SAMPLECLK )
    begin
      if ( trig_in_rise ) begin
        addr_rd_nxt <= 12'b 0000_0000_0000;
      end //else begin
      //  addr_rd_nxt <= addr_rd_nxt;
      //end
  ...
  // WRITE address increment/reset
    ...
    */
    ...
    begin
      ...
      // Moving addr_rd into the next seq_reset related if-else allows real WF data
      // but does not change that at least one 8x extra per file found

      if ( read_mem_fall ) begin     //) read_mem & new_capt ) begin // was just read_mem
        if ( addr_rd == 4 ) begin
          addr_rd <= addr_rd_nxt;
          //if ( seq_reset ) begin      // DELETED - see above - TESTING 04-26-2020 adding seq_reset here - never a chan 8x extra - but all WF data = 0x00 too ...
          //  addr_rd <= addr_rd_nxt;
          //  seq_id <= 8'h 00;
          //end else begin
          //  addr_rd <= addr_rd_nxt;
          if ( seq_reset_pending ) begin
            seq_id <= 8'h 00;
          end else begin
            seq_id <= seq_id + 1; // seq_id_nxt;
          end
          //end
          val <= seq_id;
    ...
    wire n_reset_fall;
    wire n_reset_rise;
    edge_detect edet_n_reset (
      .async_sig  (n_reset),
      .clk        (SAMPLECLK), //sclk //(clk_copy), // so far SAMPLECLK is best
      .rise       (n_reset_rise),
      .fall       (n_reset_fall)
    );
    ...
    wire debounced_seq_reset;
    wire seq_reset;
    assign seq_reset = !n_reset;    // Converting to an active high signal
    debouncing n_reset_debounce (
        .clock    (SAMPLECLK),
        .reset    (1'b 0),
        .button   (seq_reset),
        .out      (debounced_seq_reset)
        );
    ...
    // DELETED - TESTING 04-26-2020 - Testing combinatorial formulation and with = below
    // NOPE: This does the same to keep all real samples at 0x00, though never a chan 8x
    always @ ( posedge SAMPLECLK )
    begin


      // As of this moment TESTING 04-26-2020 this, below with the corresponding code in the
      // read mem always block above is the most reliable
      // So far: No 8x chans mults, and stable WF data so far anyway
      // TODO test real data on Chans to check for actual channel scanning
      // Also couples with the MCU code at the moment that always sets the n_reset
      // high at each trig, that is, after the trig has happened (so that n_reset
      // is low during a trig cycle if this is a new channel cycle) -- however
      // That breaks the first capture -- so we will try with updated MCU code
      // To indeed just drop n_reset after the trig cycle, and set it high at the
      // beginning again
      if ( seq_reset_rise ) begin // this is an active high version of the signal
        seq_reset_pending <= 1'b 1;
      end else if ( seq_id == 8'h 00 ) begin
        seq_reset_pending <= 1'b 0;
      end
      ...
      //  seq_id_nxt <= 8'h 00;
      // Yes, the formulation below matters in close timing conditions
      // In testing, so far, this formulation along with the MCU
      // timing of the RST signal provides more reliable channel reset sequence control
      // The last eval and assignment wins
      /*if ( n_reset == 1'b 1) begin
        if ( trig_in_rise ) begin
          seq_id_nxt <= seq_id + 1;
        end
      end else if ( n_reset == 1'b 0) begin
        seq_id_nxt <= 8'h 00;
        //seq_id <= 8'h 00; // can't do this ... dup input
      end*/


      // With this still get chan creep once in a while
      // And with using trig_in_rise too - which so far has been the best
      // but not perfect yet
      // Maybe try a debounce / delay?
      // What about: if ( n_reset_fall ) begin ?
      // NOPE: Using n_reset_fall - still get every couple thousand wfs a creep in of an extra 8x sequence
      // However, this last time, it was after starting capture - so it could be an MCU timing artifact - right at start of second file only place
      // Repeating: Happens again, just once, ~ sample # 20k
      // What about: if ( n_reset_rise ) begin ? Trying to reset seq count right before next capt trig_in from MCU, trying to be sure that read_mem is done
      // This assumes however that there is no overlap in read_mem completion with start of next trig cycle on MCU - so may not scale at slower read clock speeds
      // NOPE: Somehow that allows in lots of channel 8x's ... why? Oh because seq_id still holds the last number and thus is incremented at the next trig_in
      // What about if: On MCU now keep the reset signal low all the way through the channel 0 trig_in cycle
      // There is clearly a rework of all of this that could be cleaner at a next major revision
      // So then we would use not the edge pulse but just the value
      // So, trying, even the active high signal: if ( seq_reset ) along with the updated MCU code
      // This only could maybe work if we're not using a reset pulse from the MCU for anything else - which at the moment we are not - so may not scale to other options well
      // NOPE: We still get eg in file #2 one 8x extra seq
      //
      /*
      if ( seq_reset ) begin
        seq_id_nxt <= 8'h 00; // was <=
      // trig_in_rise best yet -- below leaves out a cycle of reset/chan scan - thus 08 - 0f or similar
      end else if ( trig_in_rise ) begin //or could try one of the trigd_fall/rise? // best so far but not perfect allows one or so Ch 8 every once in a while, was trig_in_rise
        seq_id_nxt <= seq_id + 1; // was <=
      end
      */


      // Or the updated variant:
      // NOPE: Let's a whole 8x chan sequence in every once in a while
      /*
      if ( !n_reset_fall) begin
        if ( trig_in_rise ) begin
          seq_id_nxt <= seq_id + 1;
        end
      end else begin  // was if ( n_reset_fall ) begin
        seq_id_nxt <= 8'h 00;
      end
      */


      // Or trying debounced active high signal
      /*
      if ( !seq_reset_rise ) begin
        if ( trig_in_rise ) begin
          seq_id_nxt <= seq_id + 1;
        end
      end else begin
...
// Seems to debounce input delaying for about 4 count cycles until high
// and then low for about 4 counts gives the low output
// So, it lasts about the input pulse duration
// However, edges are delayed by about 4 counts
// Count length is determined by N
// If input clock is 80 MHz then a count of 19 (default) => 2^18 = 262144 = 3ms (very long) or maybe x2
// We might use something very short like N = 1:0 => 2^2 (possibly 2 since it rolls over and this might be a half count trig)
// = 50 ns or maybe 100 ns
//
